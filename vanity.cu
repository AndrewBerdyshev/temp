...
std::vector<int> prefixes_count;
std::vector<char*> suffixes;
std::vector<int> suffixes_count;

// ME. Arguments parser.
bool WorkWithArguments(int argc, char const* argv[])
{
    // make copy of arguments.
    printf("%d\n", argc);
    char** arguments = (char**)malloc(argc);
    for(int i = 0; i < argc; i++)
    {
        arguments[i] = (char*)malloc(strlen(argv[i]));
        strcpy(arguments[i], argv[i]);
    }

    // For scanning list of arguments.

    for(int i = 1; i < argc; i++)
    {
        printf("%s\n", arguments[i]);
        if(!strcmp("--starts-with", arguments[i]))
        {
            if(i+1 >= argc){
...
int main(int argc, char const* argv[]) {
	// ME. Add arguments parsing.
	WorkWithArguments(argc, argv);
...
void __global__ vanity_scan(curandState* state, int* keys_found, int* gpu, int* exec_count) {
	...
	for (unsigned int n = 0; n < prefixes.size(); ++n) {
		if ( MAX_PATTERNS == n ) {
	...
	for (unsigned int n = 0; n < suffixes.size(); ++n) {
		if ( MAX_PATTERNS == n ) {
				printf("NEVER SPEAK TO ME OR MY SON AGAIN");
				return;
		}
